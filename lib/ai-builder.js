...
async function realAIGenerate(prompt) {
  ...
  html = html.replace(/^```html\s*/, "").replace(/```$/, "").trim();
  return html;
}
...
export async function generateFromPrompt(prompt, previousCode = null) {
  ...
  if (process.env.GROQ_API_KEY) {
    try {
      if (previousCode) {
        const refinePrompt = `Here is the current HTML code:\n\n${previousCode}\n\nPlease modify it according to this request: ${prompt}. Return ONLY the updated HTML code.`;
